---
type: platform
updated: 2026-03-03
---

# Bearpoint Foundation

> **Status:** Incorporated March 29, 2026 (WA Secretary of State)
> **EIN:** 41-5180172
> **Structure:** Washington Nonprofit Corporation (501(c)(3) pending)
> **Mission:** Decentralized accountability infrastructure for fighting institutional corruption. Built for Indigenous communities first, now for everyone facing institutional power.

---

## Core Model

The "medicine lodge model" — open-source legal advocacy infrastructure:

1. **Evidence Architecture:** Systematic evidence collection, cataloging, and cross-referencing
2. **Compound Pressure:** Simultaneous multi-agency, multi-jurisdiction advocacy
3. **Communications Infrastructure:** Correspondence tracking, delinquency auditing, penalty clock monitoring
4. **Tribal Sovereignty Integration:** ICWA, treaty rights, government-to-government consultation built into every workflow
5. **Replication:** Deploy-Case.ps1 — one command creates a complete advocacy vault

---

## Distribution Priority

1. Indigenous communities and tribal nations
2. MMIW advocacy organizations
3. Parents and families fighting institutional harm
4. Whistleblowers and anti-corruption advocates
5. Civil rights organizations
6. Anyone facing institutional power that counts on them not fighting back

---

## Settlement Commitment

All settlement funds from Christopherson v. YCS pledged toward indigenous healing programs.

---

## Platform Links

- [[MMIW_RESEARCH_HUB]] — MMIW research resources
- [[TOOLCHAIN]] — Available tools and setup
- [[_PROTOCOL]] — Vault rules and principles
- [[README]] — Platform overview

---

## Formation Checklist

- [ ] Articles of Incorporation (WA Secretary of State)
- [ ] EIN application (IRS SS-4)
- [ ] 501(c)(3) application (IRS Form 1023 or 1023-EZ)
- [ ] Bylaws
- [ ] Board of Directors (minimum 3)
- [ ] Registered Agent
- [ ] Trademark protection ("Bearpoint" / "Bear Point Protocol")
- [ ] Domain registration
- [ ] Banking setup (tribal-affiliated institution preferred)
